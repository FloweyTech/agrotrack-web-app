...
export class TaskApiEndpoint {
  ...
  getTasksAssignedTo(assignedToProfileId: number): Observable<Task[]> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/tasks/assigned/${assignedToProfileId}`;
  ...
  getTasksByAssignee(assigneeProfileId: number): Observable<Task[]> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/tasks/assignee/${assigneeProfileId}`;
    return this.http.get<TaskResource[]>(url).pipe(
      map((resources) => resources.map(resource => this.assembler.toEntityFromResource(resource)))
    );
  }
  ...
  createTask(task: Task): Observable<Task> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/tasks`;
  ...
  getOrganizationName(organizationId: number): Observable<string> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/organizations/${organizationId}`;
  ...
  updateTaskStatus(taskId: number, status: string): Observable<void> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/tasks/${taskId}/status`;
  ...
  deleteTask(taskId: number): Observable<void> {
    const url = `${environment.platformProviderApiBaseUrl}/api/v1/tasks/${taskId}`;
